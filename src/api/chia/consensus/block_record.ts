...
export type BlockRecord = {
  ...
  infused_challenge_vdf_output?: ClassgroupElement; // Optional[ClassgroupElement]  # This is the intermediary VDF output at ip_iters in infused cc, iff deficit <= 3
  ...
  timestamp?: uint64; // Optional[uint64]
  prev_transaction_block_hash?: bytes32; // Optional[bytes32]  # Header hash of the previous transaction block
  fees?: uint64; // Optional[uint64]
  reward_claims_incorporated?: Coin[]; // Optional[List[Coin]]
  
  // # Slot (present iff this is the first SB in sub slot)
  finished_challenge_slot_hashes?: bytes32[]; // Optional[List[bytes32]]
  finished_infused_challenge_slot_hashes?: bytes32[]; // Optional[List[bytes32]]
  finished_reward_slot_hashes?: bytes32[]; // Optional[List[bytes32]]
  
  // # Sub-epoch (present iff this is the first SB after sub-epoch)
  sub_epoch_summary_included?: SubEpochSummary; // Optional[SubEpochSummary]  
};
