...
export async function on_state_changed_of_plots(daemon: TDaemon, callback: (e: GetMessageType<chia_plotter_service, state_changed_command_of_plots, TStateChangedBroadCastOfPlots>) => unknown){
...
export async function on_message_from_chia_plots_create(daemon: TDaemon, callback: (e: GetMessageType<chia_plotter_service, chia_plots_create_commands, TChiaPlotsCreateBroadcast>) => unknown){
  await daemon.subscribe(chia_plotter_service);
  const messageListener = (e: WsMessage) => {
