...
export async function on_loaded_initial_peers(daemon: TDaemon, callback: (e: GetMessageType<chia_crawler_service, loaded_initial_peers_command, TLoadedInitialPeersBroadCast>) => unknown){
...
export async function on_crawl_batch_completed(daemon: TDaemon, callback: (e: GetMessageType<chia_crawler_service, crawl_batch_completed_command, TCrawlBatchCompletedBroadCast>) => unknown){
  await daemon.subscribe(metrics_service);
  const messageListener = (e: WsMessage) => {
